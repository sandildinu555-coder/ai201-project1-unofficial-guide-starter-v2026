...
| How is class registration priority determined based? | pass | pass | pass |
| What do students say about the quality of food in the dining hall? | fail | fail | fail |
| How do students find books for research? | pass | pass | pass |
| What scholarships and awards are available for students? | fail | fail | fail |
...
```
The housing lottery is not entirely random in the way most people assume. Rising sophomores get a number drawn at random, but juniors and seniors are ordered by accumulated credit hours first, with random selection used only for tie-breaks.
...
Source Documents: admin_housing_lottery.txt
```
...
```
Registration priority is staggered by credit hours, which is the same way the housing lottery works (advising_registration.txt).
...
```
Based on the provided documents, registration times are staggered by credit hours, similar to the housing lottery. 
...
```
Class registration priority is determined and staggered by credit hours, similar to the housing lottery. 

Source documents: advising_registration.txt
```
